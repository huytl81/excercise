...
class CustomerPortal(portal.CustomerPortal):
    ...
    @http.route(['/library_app/book-checkouts', '/library_app/book-checkouts/page/<int:page>'], auth="user", website=True)
    def my_book_checkouts(self, page=1, **kw):
        checkout = request.env["library.checkout"]
        domain = []
        # Prepare pager data
        checkout_count = checkout.search_count(domain)
        pager_data = portal.pager(url="/library_app/book_checkouts", total=checkout_count, page=page, step=self._items_per_page)
    ...
    @http.route(['/library_app/book-checkout/<model("library.checkout"):doc>'], auth="user", website=True)
