...
login = requests.post(json_endpoint, data=payload, headers=headers)
user_id = login.json()['result']
...
if user_id:
    ...
    print('Search Result:', response)  # ids will be in result keys

    # read data for books ids
    payload = get_json_payload("object", "execute_kw", db_name, user_id, password, 'library.book', 'read', [response['result'], ['name', 'published_date']])
    response = requests.post(json_endpoint, data=payload, headers=headers).json()
    print('Books data:', response)
else:
